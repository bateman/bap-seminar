...
class StackcrawlerSpider(CrawlSpider):
    ...
    start_urls = ['http://stackoverflow.com/questions?page=1&sort=featured']

    rules = (
        Rule(LinkExtractor(allow=r'\/questions\?page=[1-5]\&(amp;)?sort=featured'), callback='parse_page',
             follow=False),
    )

    def parse_page(self, response):
        sel = Selector(text=response.body, type="html")
        threads_urls = sel.xpath("//h3/a[@class='question-hyperlink']/@href").extract()
    ...
    def parse_thread(self, response):
        ...
        if answers_count > 0:
            for i in range(1, answers_count + 1):
                ...
                try:
                    answer['accepted'] = response.xpath(
                        "//div[@id='answers']//div[@id='answer-{0}']/@itemprop".format(answer['id'])).extract()[0]
